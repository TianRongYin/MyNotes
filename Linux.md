...
- 基本命令
  ...
  - 文件/文件夹操作:
    ...
    - 移动/改名：`mv 参数一 参数二`移动文件/文件夹，如果目标不存在则将移动文件改名
...
- `top`命令查看CPU、内存使用情况，相当于Windows的任务管理器，按q或CTRL+C退出，[详请见附录](#TOP命令详解)
- `df [-h]`查看硬盘的使用情况，`-h`更人性化
- `iostat [-x] [num1] [num2]`查看磁盘cpu占用情况
  - `-x`显示，`num1`刷新间隔，`num2`刷新几次，[指标](#iostat指标)
- `sar -n DEV [num1] [num2]`网络监控，由于sar命令十分复杂，这里仅作简单统计
  - `-n`查看网络，DEV查看网络接口

### 环境变量

- 概述
  - 环境变量是一组信息记录，类型是KeyValue型，记录一些关键信息辅助系统的运行
  - 我们使用的一系列命令其实都是一些可执行文件，比如`cd`命令的执行程序在`/user/bin/cd`目录，无论我们在哪个工作目录输入命令，操作系统都会在环境变量`PATH`中查找对应的可执行文件
  - `env`查看当前系统的所有环境变量
  - PATH环境变量下有多个路径通过`:`隔开，输入命令操作系统则会在PATH的目录中一个一个找
  - `${KEY}`可以取出环境变量中Value的值
- 自行设置环境变量
  - 临时：`export 变量名=变量值`
  - 永久：
    - 针对当前用户：编辑`~/.bashrc`，在文件内添加`export 变量名=变量值`
    - 针对所有用户：编辑`/etc/profile`，在文件内添加`export 变量名=变量值`
    - 通过语法`source 配置文件路径`使其生效
- 自定义环境变量PATH
  - 创建文件，并在文件中写入命令
  - 添加执行权限（`chmod`）就可以用`./文件名`在当前工作目录执行文件
  - 用root权限修改`/etc/profile`文件
  - 在行尾添加`export PATH=$PATH:文件路径`：在原有PATH的基础上添加环境变量
  - `source`生效即可在任何目录调用该命令

### 上传下载与解压压缩

- 上传下载：

  - FinalShell直接拖拽即可进行上传，选择文件点击下载
    - 需要在FinalShell中配置用户名为root登录，命令行切换用户无效

  - 命令行：
    - 需要安装lrzsz：`yum -y install lrzsz`
      - `sz 文件路径`即可下载
      - `rz`直接回车即可选择文件上传

- 解压压缩：

  - `.tar`	：仅仅封装，无压缩效果；`.gz`：使用gzip算法压缩后的压缩文件格式，极大减少体积

    - `tar [-c -v -x -f -z -C] 压缩包名 参数一···参数n`

      | -c       | -v       | -x       | -f                             | -z                   | -C         |
      | -------- | -------- | -------- | ------------------------------ | -------------------- | ---------- |
      | 压缩模式 | 显示进度 | 解压模式 | 后接参数文件，必须处于最后一个 | gzip模式，通常在首部 | 解压目的地 |

    - 压缩示例：tar:`tar -cvf test.tar 1.txt 2.txt 3.txt`、gzip:`tar -zcvf test.tar.gz 1.txt 2.txt 3.txt`

    - 解压示例：`tar -xvf test.tar -C / home/ytr`、`tar -zxvf test.tar.gz`

  - `.zip`：zip压缩包

    - `zip [-r] 压缩包名 参数一···参数n`压缩文件，被压缩的文件中包含文件夹时要加`-r`参数
    - `unzip 压缩包名 [-d] 参数`解压文件，`-d`解压到参数目录
